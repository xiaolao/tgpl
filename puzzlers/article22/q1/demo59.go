...
type handlerConfig struct {
	...
	interval  time.Duration // 单个goroutine处理的时间间隔
	counter   int           // 数量计数器,以字节为单位。
	counterMu sync.Mutex    // 数量计数器专用的互斥锁
}
...
func main() {
	...
	// 信号通道, goroutine之间最好使用channel通信
	var sign = make(chan struct{}, writingConfig.goNum+readingConfig.goNum)

	// 启用多个goroutine对缓冲区进行多次写操作
	...
	// 启动多个goroutine对缓冲区进行多次数据读取
	for i := 1; i <= readingConfig.goNum; i++ {
		go func(i int) {
			defer func() {
				sign <- struct{}{}
			}()
			for j := 1; j <= readingConfig.goNum; j++ {
