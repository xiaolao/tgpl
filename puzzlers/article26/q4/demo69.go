...
func main() {
	...
	fmt.Printf("The value of key %v found in the node3: %v\n",
		keys[0], node3.Value(keys[0])) // 20  value in node2
	fmt.Printf("The value of key %v found in the node3: %v\n",
		keys[1], node3.Value(keys[1])) // 30 value in node3
	fmt.Printf("The value of key %v found in the node3: %v\n",
		keys[2], node3.Value(keys[2])) // 60 nil
	fmt.Println()

	// example 2.
	node4, _ := context.WithCancel(node3)
	node5, _ := context.WithTimeout(node4, time.Hour)
	fmt.Printf("The value of key %v found in the node5: %v\n",
		keys[0], node5.Value(keys[0])) // 20   value in node2
	fmt.Printf("The value of key %v found in the node5: %v\n",
		keys[0], node5.Value(keys[0])) // 20   value in node2
	fmt.Println()
